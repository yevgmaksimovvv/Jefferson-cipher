...
## Что не описывает этот документ
* не является API reference;
* не заменяет `docs/runtime.md`;
* не описывает пользовательский интерфейс подробно;
* не является ADR-журналом.

> [!TIP]
> Значимые архитектурные решения лучше фиксировать отдельными ADR-документами, если проект начнёт активно менять границы слоёв.
